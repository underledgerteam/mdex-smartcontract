...
describe('TEST BEST RATE', () => {
  ...
  beforeEach(async () => {
    ...
    const MdexBestRateQuery = await ethers.getContractFactory(
      'MdexBestRateQuery',
    );
    mdexBestRateQuery = await MdexBestRateQuery.deploy(mdexController.address);
  ...
  it('Use Case #2 : Should should spilt swap', async () => {
    const data = await mdexBestRateQuery.splitTwoRoutes(
      tokenA.address,
      tokenB.address,
