...
describe("TEST UNISWAP POOL", () => {
  ...
  it("User Case #3 : Should Get Zero Address", async () => {
    ...
    expect(
      await factory.getPair(tokenA.address, token3.address))
  ...
  it("User Case #4 : Should Add Liquidity", async () => {
    ...
    expect(await tokenA.balanceOf(user1.address)).to.equal(initialToken - 100000);
    expect(await tokenB.balanceOf(user1.address)).to.equal(initialToken - 100000);
  });
  ...
  it("User Case #5 : Should Swap A to B", async () => {
    ...
    amountOutMin = await router.getAmountOut(amountIn, reserveBefore[0], reserveBefore[1]);
    
    // user2 swap tokenA -> tokenB
    await router
      .connect(user2)
      .swapExactTokensForTokens(
        amountIn, amountOutMin, [tokenA.address, tokenB.address], user2.address, maxUint
      );
  ...
  it("User Case #6 : Should Swap B to A", async () => {
    ...
    await router
      .connect(user2)
      .swapExactTokensForTokens(
        amountIn, amountOutMin, [tokenB.address, tokenA.address], user2.address, maxUint
      );
